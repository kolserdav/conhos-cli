# Hosting Ruby with Mariadb

To host `Ruby` on Container Hosting and connect to the `Mariadb` database from it, you need to complete the following three steps.
...
Configuration file for creating a `Ruby` service in Container Hosting with a `Mariadb` database server running and connecting to it from an application, and an optional example of updating `Adminer` to administer the database. More details in the [Configuration File](./ConfigFile.md#example_configuration_file).
...
[Hosting Ruby Mysql <<<](./HostingRubyMysql.md) | [>>> Hosting Ruby Mongo](./HostingRubyMongo.md)
