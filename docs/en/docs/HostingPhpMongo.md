...
Configuration file for creating a `Php` service in Container Hosting with a `Mongo` database server running and connecting to it from an application, and an optional example of updating `Adminer` to administer the database. More details in the [Configuration File](./ConfigFile.md#example_configuration_file).
...
<div style="display: flex; flex-direction: row; justify-content: space-around;"><span>[Hosting Php Mariadb <<<](./HostingPhpMariadb.md)</span> <span>|</span> <span>[>>> Hosting Php Rabbitmq](./HostingPhpRabbitmq.md)</span></div>
