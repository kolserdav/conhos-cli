# Hosting Ruby with Postgres

To host `Ruby` on Container Hosting and connect to the `Postgres` database from it, you need to complete the following three steps.
...
Configuration file for creating a `Ruby` service in Container Hosting with a `Postgres` database server running and connecting to it from an application, and an optional example of updating `Adminer` to administer the database. More details in the [Configuration File](./ConfigFile.md#example_configuration_file).
...
[Hosting Ruby Redis <<<](./HostingRubyRedis.md) | [>>> Hosting Ruby Mysql](./HostingRubyMysql.md)
