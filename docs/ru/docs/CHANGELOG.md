...
- `[Ядро]` Инфраструктура системы перенесена с Docker swarm на Kubernetes ...
- `[Сервер]` Исправлено преждевременное удаление гит прослушивателя, когда приходило уведомление `Невозможно подключиться к Git` если github временно недоступен
